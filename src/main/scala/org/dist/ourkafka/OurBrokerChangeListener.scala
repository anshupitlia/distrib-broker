...
class OurBrokerChangeListener(zkClient: OurZookeperClient) extends IZkChildListener with Logging  {
  ...
  override def handleChildChange(parentPath: String, currentBrokerList: util.List[String]): Unit = {
    ...
    try {
      ...
      val newBrokers = newBrokerIds.map(b => zkClient.getBrokerInfo(b))
